...
- [Frontend Mentor - Character counter solution](#frontend-mentor---character-counter-solution)
  ...
  - [Highlights from my solution](#highlights-from-my-solution)
    - [Files I updated while working on the theme toggle](#files-i-updated-while-working-on-the-theme-toggle)
...
### Files I updated while working on the theme toggle

- `starter-code/assets/css/styles.css` — added theme-aware variables and styles, fixed a toggler selector.
- `starter-code/js/script.js` — added an accessible theme toggle, icon/logo swapping, initialization from saved preference or system preference, and persistence in localStorage.
 - `starter-code/assets/css/styles.css` — added custom checkbox styles (uses `input + label::before` and `assets/images/icon-check.svg`) and theme-aware styling.

### How to test locally

1. From the `starter-code` directory run a simple static server (Python 3):

```powershell
cd 'C:\Users\Joe\Desktop\character_counter\starter-code'
